...
void trainNN(double* train_data, double* train_one_hot_labels, double* train_labels, double* val_data, double* val_labels, double* val_one_hot_labels,double* test_data, double* test_one_hot_labels, double* test_labels, double* firstHiddenLayerWeight, double *secondHiddenLayerWeight, double *lastHiddenLayerWeight, double* firstBiases, double *secondBiases, double *lastBiases, int num_epoch, int numberOfTrainImage, int numberOfValImage, int numberOfTestImage, int inputCols, int firstHiddenLayerSize, int secondHiddenLayerSize, int lastHiddenLayerSize) {
    ...
    forwardNN(test_data, firstHiddenLayerWeight, firstBiases, firstLayerResult, numberOfTestImage, inputCols, firstHiddenLayerSize);
    forwardNN(firstLayerResult, secondHiddenLayerWeight, secondBiases, secondLayerResult, numberOfTestImage, firstHiddenLayerSize, secondHiddenLayerSize);
    forwardNN(secondLayerResult, lastHiddenLayerWeight, lastBiases, lastLayerResult, numberOfTestImage, secondHiddenLayerSize, lastHiddenLayerSize, false);

    softmax(lastLayerResult, numberOfTestImage, lastHiddenLayerSize);

    cout <<"Test Accuracy: " << accuracy(lastLayerResult, test_labels, numberOfTestImage, lastHiddenLayerSize) << \
        ", Test Loss: " << crossEntropy(lastLayerResult, test_one_hot_labels, numberOfTestImage, lastHiddenLayerSize) << endl;
...
int main() {
    ...
    string testFileName = "data/t10k-images-idx3-ubyte";
    string testLabelFileName = "data/t10k-labels-idx1-ubyte";
    ...
    unsigned int number_of_test_images, n_test_rows, n_test_cols; 
    ...
    read_mnist(testFileName, test_data, number_of_test_images, n_test_rows, n_test_cols);
    ...
    unsigned int requiredMemsizeForTestLabel = number_of_test_images * 10;
    ...
    test_one_hot_labels = (double*)malloc(requiredMemsizeForTestLabel * sizeof(double));
    ...
    test_labels = (double*)malloc(number_of_test_images * sizeof(double));

    read_labels(nameOfLabelFile, input_labels);
    read_labels(valLabelFileName, val_labels);
    read_labels(testLabelFileName, test_labels);
    ...
    GpuTimer timer;
    timer.Start();
    trainNN(input_data, input_one_hot_labels, input_labels, val_data, val_labels, val_one_hot_labels, test_data, test_one_hot_labels, test_labels, firstHiddenLayerWeight, secondHiddenLayerWeight, lastHiddenLayerWeight, firstBiases, secondBiases, lastBiases, NUM_EPOCH, number_of_train_images, number_of_val_images, number_of_test_images, inputLayerSize, firstHiddenLayerSize, secondHiddenLayerSize, lastHiddenLayerSize);
    timer.Stop();
    cout << "Thoi gian de huan luyen, validate va test la: " << timer.Elapsed() << " ms" << endl;
    //##############################################################################################################################################
